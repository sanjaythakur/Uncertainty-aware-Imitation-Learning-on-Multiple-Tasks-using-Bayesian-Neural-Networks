...
def run_policy(env, policy, scaler, logger, episodes):
    ...
    for e in range(episodes):
        ...
        trajectory = {DEMONSTRATOR_OBSERVATIONS_KEY: observes,
    ...
    unscaled = np.concatenate([t[DEMONSTRATOR_UNSCALED_OBSERVATIONS_KEY] for t in trajectories])
...
def main(env_name, num_episodes, gamma, lam, kl_targ, batch_size, hid1_mult, policy_logvar, task_identity):
    ...
    trajectories = run_policy(env, policy, scaler, logger, episodes=DEMONSTRATOR_EPISODES_TO_LOG)
    data_to_store = {DEMONSTRATOR_TRAJECTORY_KEY: trajectories, SCALE_KEY: scale, OFFSET_KEY: offset}
